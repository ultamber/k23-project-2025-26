...
void Hypercube::buildIndex() {
    ...
    // Generate bit-mapping functions f_j: Z → {0,1}
    ...
    // Choose storage strategy based on d'
...
/**
 * Computes h_j(v) = floor((a_j · v + t_j) / w) for projection j
...
/**
 * Maps hash value to bit using function f_j
 * @param h Hash value
 * @param j Projection index
...
uint64_t Hypercube::vertexOf(const vector<float> &v) const {
    uint64_t key = 0;
    for (int j = 0; j < kproj_; ++j)
        if (fj(hij(v, j), j))               // Map to bit using f_j
            key |= (1ULL << j);
    return key;
}

/**
 * Generates vertices to probe in order of increasing Hamming distance
 ...
 * @param maxHamming Maximum Hamming distance to probe
 * @return Vector of vertex labels to probe
 */
vector<uint64_t> Hypercube::probesList(uint64_t base, int kproj, int maxProbes, int maxHamming) const {
...
void Hypercube::search(const vector<VectorData> &queries, ofstream &out) {
    ...
    for (int qi = 0; qi < Q; ++qi) {
        ...
        // Project query to hypercube vertex
        uint64_t base = vertexOf(q);

        // Check points in same and nearby vertices
        // Generate vertices in increasing Hamming distance
        auto plist = probesList(base, kproj_, Args.probes, maxHam);

        // Collect unique candidates from probed vertices
        unordered_set<int> candSet;
        candSet.reserve(min<size_t>((size_t)Args.M, (size_t)4096));
        size_t gathered = 0;
        ...
        for (auto vtx : plist) {
            ...
            for (int id : *bucket)
                if (candSet.insert(id).second && ++gathered >= (size_t)Args.M)
                    break;

            if (gathered >= (size_t)Args.M)
                break;
        }
        ...
        // Return closest candidates or range search results
        int topN = min(Args.N, (int)distApprox.size());
        if (topN > 0) {
